...
export const UpdateCompanyModal = ({ open, row, onClose, onSubmitModal }) => {
  ...
  useEffect(() => {
    ...
    setFieldValue('companyLabel', row.companyLabel);
    setFieldValue('companyDescription', row.companyDescription);
    setFieldValue('companyAddress', row.companyAddress);
    setFieldValue('companyPhoneNumber', row.companyPhoneNumber);
    const photo = row.companyLogo ? BASE_URL + row.companyLogo.split('\\')[1] : '';
